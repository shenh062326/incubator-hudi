...
public class TestHoodieBloomIndex extends HoodieClientTestHarness {
  ...
  public void testRangePruning() {
    ...
    List<String, String> partitionRecordKeyPairRDD =
    ...
    List<Tuple2<String, HoodieKey>> comparisonKeyList =
        index.explodeRecordRDDWithFileComparisons(partitionToFileIndexInfo, partitionRecordKeyPairRDD);
  ...
  public void testCheckUUIDsAgainstOneFile() throws IOException, InterruptedException {
    ...
    List<String> results = HoodieKeyLookupHandle.checkCandidatesAgainstFile(conf, uuids,
  ...
  public void testTagLocationWithEmptyRDD() {
    // We have some records to be tagged (two different partitions)
    List<HoodieRecord> recordRDD = new ArrayList<>();
  ...
  public void testTagLocation() throws Exception {
    ...
    HoodieRecord record4 =
        new HoodieRecord(new HoodieKey(rowChange4.getRowKey(), rowChange4.getPartitionPath()), rowChange4);
    List<HoodieRecord> recordRDD = Arrays.asList(record1, record2, record3, record4);
    ...
    // Let's tag
    HoodieBloomIndex bloomIndex = new HoodieBloomIndex(config);
    List<HoodieRecord> taggedRecordRDD = bloomIndex.tagLocation(recordRDD, table);

    // Should not find any files
    for (HoodieRecord record : taggedRecordRDD) {
  ...
  public void testBloomFilterFalseError() throws IOException, InterruptedException {
    ...
    // We do the tag
    List<HoodieRecord> recordRDD = Arrays.asList(record1, record2);
    HoodieWriteConfig config = makeConfig();
    metaClient = HoodieTableMetaClient.reload(metaClient);
    HoodieTable table = HoodieTable.getHoodieTable(metaClient, config, conf);
