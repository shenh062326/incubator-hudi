...
public class TestHoodieClientBase extends HoodieClientTestHarness {
  ...
  static final Logger LOG = LogManager.getLogger(TestHoodieClientBase.class);
  ...
  protected HoodieCleanClient getHoodieCleanClient(HoodieWriteConfig cfg) {
    return new HoodieCleanClient(conf, cfg, new HoodieMetrics(cfg, cfg.getTableName()));
  }
  ...
  protected HoodieWriteClient getHoodieWriteClient(HoodieWriteConfig cfg, boolean rollbackInflightCommit,
                                                   HoodieIndex index) {
    return new HoodieWriteClient(conf, cfg, rollbackInflightCommit, index);
  }

  protected HoodieReadClient getHoodieReadClient(String basePath) {
    return new HoodieReadClient(conf, basePath);
  }
  ...
  private Function2<List<HoodieRecord>, String, Integer> wrapRecordsGenFunctionForPreppedCalls(
    ...
    return (commit, numRecords) -> {
      ...
      final HoodieTableMetaClient metaClient = new HoodieTableMetaClient(conf, basePath, true);
      HoodieTable table = HoodieTable.getHoodieTable(metaClient, writeConfig, conf);
      List<HoodieRecord> taggedRecords = index.tagLocation(records, table);
  ...
  private Function<Integer, List<HoodieKey>> wrapDeleteKeysGenFunctionForPreppedCalls(
    ...
    return (numRecords) -> {
      ...
      List<HoodieRecord> recordsToDelete = records.stream().map(
          key -> new HoodieRecord(key, new EmptyHoodieRecordPayload())).collect(Collectors.toList());
      List<HoodieRecord> taggedRecords = index.tagLocation(recordsToDelete, table);
   ...
   */
  List<WriteStatus> insertFirstBatch(HoodieWriteConfig writeConfig, HoodieWriteClient client, String newCommitTime,
                                        String initCommitTime, int numRecordsInThisCommit,
                                        Function3<List<WriteStatus>, HoodieWriteClient, List<HoodieRecord>, String> writeFn, boolean isPreppedAPI,
   ...
   * @throws Exception in case of error
   */
  List<WriteStatus> updateBatch(HoodieWriteConfig writeConfig, HoodieWriteClient client, String newCommitTime,
   ...
   * @throws Exception in case of error
   */
  List<WriteStatus> deleteBatch(HoodieWriteConfig writeConfig, HoodieWriteClient client, String newCommitTime,
                                   String prevCommitTime, String initCommitTime,
                                   int numRecordsInThisCommit,
                                   Function3<List<WriteStatus>, HoodieWriteClient, List<HoodieKey>, String> deleteFn, boolean isPreppedAPI,
...
/**
   ...
   * @throws Exception in case of error
   */
  List<WriteStatus> writeBatch(HoodieWriteClient client, String newCommitTime, String prevCommitTime,
                                  Option<List<String>> commitTimesBetweenPrevAndNew, String initCommitTime, int numRecordsInThisCommit,
                                  Function2<List<HoodieRecord>, String, Integer> recordGenFunction,
                                  Function3<List<WriteStatus>, HoodieWriteClient, List<HoodieRecord>, String> writeFn,
                                  boolean assertForCommit, int expRecordsInThisCommit, int expTotalRecords, int expTotalCommits) throws Exception {
    ...
    if (assertForCommit) {
      ...
      assertEquals("Incremental consumption from " + prevCommitTime + " should give all records in latest commit",
          HoodieClientTestUtils.readCommit(basePath, timeline, newCommitTime).size(),
          HoodieClientTestUtils.readSince(basePath, timeline, prevCommitTime).size());
      if (commitTimesBetweenPrevAndNew.isPresent()) {
        commitTimesBetweenPrevAndNew.get().forEach(ct -> {
          assertEquals("Incremental consumption from " + ct + " should give all records in latest commit",
              HoodieClientTestUtils.readCommit(basePath, timeline, newCommitTime).size(),
              HoodieClientTestUtils.readSince(basePath, timeline, ct).size());
        });
      }
    }
    return statuses;
   ...
   * @throws Exception in case of error
   */
  List<WriteStatus> deleteBatch(HoodieWriteClient client, String newCommitTime, String prevCommitTime,
                                   String initCommitTime, int numRecordsInThisCommit,
                                   Function<Integer, List<HoodieKey>> keyGenFunction,
                                   Function3<List<WriteStatus>, HoodieWriteClient, List<HoodieKey>, String> deleteFn,
                                   boolean assertForCommit, int expRecordsInThisCommit, int expTotalRecords) throws Exception {
    ...
    List<WriteStatus> statuses = deleteFn.apply(client, deleteRecords, newCommitTime);
    assertNoWriteErrors(statuses);
