...
public class TestMultiFS extends HoodieClientTestHarness {
  ...
  public void readLocalWriteHDFS() throws Exception {
    ...
    HoodieTableMetaClient.initTableType(conf, dfsBasePath, HoodieTableType.valueOf(tableType),
    ...
    try (HoodieWriteClient hdfsWriteClient = getHoodieWriteClient(cfg);
      ...
      List<GenericRecord> readRecords = HoodieClientTestUtils.readCommit(dfsBasePath, timeline, readCommitTime);
      assertEquals("Should contain 100 records", readRecords.size(), records.size());

      // Write to local
      HoodieTableMetaClient.initTableType(conf, tablePath, HoodieTableType.valueOf(tableType),
          tableName, HoodieAvroPayload.class.getName());
      ...
      List<HoodieRecord> localWriteRecords = localRecords;
      ...
      List<GenericRecord> localReadRecords =
          HoodieClientTestUtils.readCommit(tablePath, timeline, writeCommitTime);
      assertEquals("Should contain 100 records", localReadRecords.size(), localRecords.size());
    }
  }
}
