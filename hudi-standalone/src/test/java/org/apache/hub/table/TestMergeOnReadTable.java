...
import org.apache.hub.HoodieReadClient;
...
import org.apache.hub.common.HoodieMergeOnReadTestUtils;
...
public class TestMergeOnReadTable extends HoodieClientTestHarness {
  ...
  public void init() throws IOException {
    initDFS();
    conf.addResource(dfs.getConf());
    initPath();
    dfs.mkdirs(new Path(basePath));
    HoodieTestUtils.init(conf, basePath, HoodieTableType.MERGE_ON_READ);
  ...
  public void testSimpleInsertAndUpdate() throws Exception {
    ...
    try (HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      List<WriteStatus> statuses = client.upsert(writeRecords, newCommitTime);
      assertNoWriteErrors(statuses);

      HoodieTableMetaClient metaClient = new HoodieTableMetaClient(conf, cfg.getBasePath());
      HoodieTable hoodieTable = HoodieTable.getHoodieTable(metaClient, cfg, conf);
      ...
      statuses = client.upsert(records, newCommitTime);
      ...
      assertEquals("Must contain 200 records", 200,
          HoodieClientTestUtils.readSince(basePath, timeline, "000").size());
    }
  }
  ...
  public void testSimpleInsertUpdateAndDelete() throws Exception {
    ...
    try (HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      statuses = client.upsert(fewRecordsForDelete, newCommitTime);
      // Verify there are no errors
      assertNoWriteErrors(statuses);
  ...
  public void testCOWToMORConvertedTableRollback() throws Exception {
    ...
    HoodieTestUtils.init(conf, basePath, HoodieTableType.COPY_ON_WRITE);
  ...
  public void testRollbackWithDeltaAndCompactionCommit() throws Exception {
    ...
    try (HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      List<WriteStatus> writeStatusList = client.upsert(writeRecords, newCommitTime);
      client.commit(newCommitTime, writeStatusList);
      List<WriteStatus> statuses = writeStatusList;
      assertNoWriteErrors(statuses);
      ...
      try (HoodieWriteClient secondClient = getWriteClient(getHoodieWriteConfigWithSmallFileHandlingOff());) {
        ...
        statuses = secondClient.upsert(copyOfRecords, commitTime1);
        // Verify there are no errors
        assertNoWriteErrors(statuses);
      ...
      try (HoodieWriteClient thirdClient = getWriteClient(cfg);) {
        ...
        writeRecords = copyOfRecords;
        writeStatusList = thirdClient.upsert(writeRecords, commitTime2);
        thirdClient.commit(commitTime2, writeStatusList);
        statuses = writeStatusList;
        // Verify there are no errors
        assertNoWriteErrors(statuses);
        ...
        metaClient = HoodieTableMetaClient.reload(metaClient);
        hoodieTable = HoodieTable.getHoodieTable(metaClient, cfg, conf);
        roView = new HoodieTableFileSystemView(metaClient, hoodieTable.getCompletedCommitsTimeline(), allFiles);
        dataFiles = roView.getLatestBaseFiles().map(HoodieBaseFile::getPath).collect(Collectors.toList());
        recordsRead = HoodieMergeOnReadTestUtils.getRecordsUsingInputFormat(dataFiles, basePath);
        ...
        List<HoodieRecord> copyOfRecords1 = new ArrayList<>(records);
        copyOfRecords1 = dataGen.generateUpdates(commitTime2, copyOfRecords1);
        copyOfRecords1.addAll(dataGen.generateInserts(commitTime2, 200));
        writeRecords = copyOfRecords1;
        writeStatusList = thirdClient.upsert(writeRecords, newCommitTime);
        thirdClient.commit(newCommitTime, writeStatusList);
        statuses = writeStatusList;
        // Verify there are no errors
        assertNoWriteErrors(statuses);
        ...
        List<WriteStatus> ws = thirdClient.compact(compactionInstantTime);
  ...
  public void testMultiRollbackWithDeltaAndCompactionCommit() throws Exception {
    ...
    try (final HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      statuses = nClient.upsert(copyOfRecords, newCommitTime);
      // Verify there are no errors
      assertNoWriteErrors(statuses);
      nClient.commit(newCommitTime, writeStatusList);
      copyOfRecords.clear();
      ...
      writeStatusList = client.upsert(writeRecords, newCommitTime);
      client.commit(newCommitTime, writeStatusList);
      statuses = writeStatusList;
      // Verify there are no errors
      assertNoWriteErrors(statuses);
      ...
      client.scheduleCompactionAtInstant(compactionInstantTime, Option.empty());
      List<WriteStatus> ws = client.compact(compactionInstantTime);
      ...
      statuses = client.upsert(copyOfRecords, newCommitTime);
      // Verify there are no errors
      assertNoWriteErrors(statuses);
      client.commit(newCommitTime, writeStatusList);
      copyOfRecords.clear();
  ...
  public void testUpsertPartitioner() throws Exception {
    ...
    try (HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      statuses = client.upsert(newRecords, newCommitTime);
      // Verify there are no errors
      assertNoWriteErrors(statuses);
  ...
  public void testLogFileCountsAfterCompaction() throws Exception {
    ...
    try (HoodieWriteClient writeClient = getWriteClient(config);) {
      ...
      try (HoodieReadClient readClient = new HoodieReadClient(conf, config);) {
        updatedRecords = readClient.tagLocation(updatedRecordsRDD);
        ...
        List<WriteStatus> result = writeClient.compact(compactionInstantTime);
  ...
  public void testSimpleInsertsGeneratedIntoLogFiles() throws Exception {
    ...
    try (HoodieWriteClient writeClient = getWriteClient(config);) {
      ...
      HoodieTable table =
          HoodieTable.getHoodieTable(new HoodieTableMetaClient(conf, basePath), config, conf);
      ...
      assertEquals(statuses.stream().map(status -> status.getStat().getPath().contains("parquet")).count(), numLogFiles);
      Assert.assertEquals(statuses.size(), numLogFiles);
  ...
  public void testInsertsGeneratedIntoLogFilesRollback() throws Exception {
    ...
    try (HoodieWriteClient writeClient = getWriteClient(config);) {
      ...
      List<WriteStatus> statuses = writeClient.insert(recordsRDD, newCommitTime);
      // trigger an action
      List<WriteStatus> writeStatuses = statuses;
  ...
  public void testInsertsGeneratedIntoLogFilesRollbackAfterCompaction() throws Exception {
    ...
    try (HoodieWriteClient writeClient = getWriteClient(config);) {
      ...
      writeClient.rollback(newCommitTime);
      table = HoodieTable.getHoodieTable(new HoodieTableMetaClient(conf, basePath), config, conf);
  ...
  public void testRollingStatsInMetadata() throws Exception {
    ...
    try (HoodieWriteClient client = getWriteClient(cfg);) {
      ...
      List<WriteStatus> statuses = client.insert(writeRecords, commitTime);
      assertTrue("Commit should succeed", client.commit(commitTime, statuses));

      // Read from commit file
      table = HoodieTable.getHoodieTable(metaClient, cfg, conf);
