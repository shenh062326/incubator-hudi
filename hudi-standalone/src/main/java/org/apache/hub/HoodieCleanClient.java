...
public class HoodieCleanClient<T extends HoodieRecordPayload> extends AbstractHoodieClient {
  ...
  public HoodieCleanClient(Configuration conf, HoodieWriteConfig clientConfig, HoodieMetrics metrics) {
    this(conf, clientConfig, metrics, Option.empty());
  }
  ...
  protected HoodieCleanMetadata clean(String startCleanTime) throws HoodieIOException {
    ...
    final HoodieTable<T> table = HoodieTable.getHoodieTable(createMetaClient(true), config, conf);
    ...
    if (cleanerPlanOpt.isPresent()) {
      ...
      if ((cleanerPlan.getFilesToBeDeletedPerPartition() != null)
        ...
        final HoodieTable<T> hoodieTable =
            HoodieTable.getHoodieTable(createMetaClient(true), config, conf);
  ...
  protected Option<HoodieCleanerPlan> scheduleClean(String startCleanTime) {
    ...
    HoodieCleanerPlan cleanerPlan = table.scheduleClean(conf);
  ...
  private HoodieCleanMetadata runClean(HoodieTable<T> table, HoodieInstant cleanInstant,
    ...
    try {
      ...
      List<HoodieCleanStat> cleanStats = table.clean(conf, cleanInstant, cleanerPlan);
