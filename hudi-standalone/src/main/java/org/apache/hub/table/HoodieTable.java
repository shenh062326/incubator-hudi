...
import org.apache.hub.client.utils.ClientUtils;
...
import org.apache.hub.exception.HoodieSavepointException;
...
public abstract class HoodieTable<T extends HoodieRecordPayload> implements Serializable {
  ...
  protected HoodieTable(HoodieWriteConfig config, Configuration conf) {
    this.config = config;
    this.hadoopConfiguration = new SerializableConfiguration(conf);
    this.viewManager = FileSystemViewManager.createViewManager(new SerializableConfiguration(conf),
        config.getViewStorageConfig());
    this.metaClient = ClientUtils.createMetaClient(conf, config, true);
  ...
  public static <T extends HoodieRecordPayload> HoodieTable<T> getHoodieTable(HoodieTableMetaClient metaClient,
      HoodieWriteConfig config, Configuration conf) {
    switch (metaClient.getTableType()) {
      case COPY_ON_WRITE:
        return new HoodieCopyOnWriteTable<>(config, conf);
      case MERGE_ON_READ:
        return new HoodieMergeOnReadTable<>(config, conf);
      default:
   ...
   * @return
   */
  public abstract HoodieCompactionPlan scheduleCompaction(Configuration conf, String instantTime);
   ...
   */
  public abstract List<WriteStatus> compact(Configuration conf, String compactionInstantTime,
   ...
   */
  public abstract HoodieCleanerPlan scheduleClean(Configuration conf);
   ...
   */
  public abstract List<HoodieCleanStat> clean(Configuration conf, HoodieInstant cleanInstant,
   ...
   */
  public abstract List<HoodieRollbackStat> rollback(Configuration conf, HoodieInstant instant, boolean deleteInstants)
      throws IOException;
   ...
   */
  public void finalizeWrite(String instantTs, List<HoodieWriteStat> stats)
  ...
  protected void cleanFailedWrites(String instantTs, List<HoodieWriteStat> stats,
    ...
    try {
      ...
      if (!groupByPartition.isEmpty()) {
        ...
        if (consistencyCheckEnabled) {
          // This will either ensure all files to be deleted are present.
          waitForAllFiles(groupByPartition, FileVisibility.APPEAR);
        }

        // Now delete partially written files
        new ArrayList<>(groupByPartition.values()).stream()
            .map(partitionWithFileList -> {
              final FileSystem fileSystem = metaClient.getFs();
              LOG.info("Deleting invalid data files=" + partitionWithFileList);
              if (partitionWithFileList.isEmpty()) {
        ...
        if (consistencyCheckEnabled) {
          // This will either ensure all files to be deleted are absent.
          waitForAllFiles(groupByPartition, FileVisibility.DISAPPEAR);
        }
      }
      // Now delete the marker directory
      deleteMarkerDir(instantTs);
   ...
   */
  private void waitForAllFiles(Map<String, List<Pair<String, String>>> groupByPartition,
                               FileVisibility visibility) {
    // This will either ensure all files to be deleted are present.
    boolean checkPassed =
        new ArrayList<>(groupByPartition.entrySet()).stream()
            .allMatch(partitionWithFileList -> waitForCondition(partitionWithFileList.getKey(),
