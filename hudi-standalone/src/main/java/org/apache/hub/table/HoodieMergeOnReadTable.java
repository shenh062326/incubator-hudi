...
import org.apache.hub.exception.HoodieCompactionException;
import org.apache.hudi.exception.HoodieIOException;
import org.apache.hub.exception.HoodieUpsertException;
import org.apache.hub.func.MergeOnReadLazyInsertIterable;
import org.apache.hub.io.HoodieAppendHandle;
import org.apache.hub.io.compact.HoodieMergeOnReadTableCompactor;
...
public class HoodieMergeOnReadTable<T extends HoodieRecordPayload> extends HoodieCopyOnWriteTable<T> {
  ...
  public HoodieMergeOnReadTable(HoodieWriteConfig config, Configuration conf) {
    super(config, conf);
  }
  ...
  @Override
  public HoodieCompactionPlan scheduleCompaction(Configuration conf, String instantTime) {
    ...
    try {
      return compactor.generateCompactionPlan(conf, this, config, instantTime,
  ...
  public List<WriteStatus> compact(Configuration conf, String compactionInstantTime,
    ...
    HoodieMergeOnReadTableCompactor compactor = new HoodieMergeOnReadTableCompactor();
    try {
      return compactor.compact(conf, compactionPlan, this, config, compactionInstantTime);
    } catch (IOException e) {
  ...
  @Override
  public List<HoodieRollbackStat> rollback(Configuration conf, HoodieInstant instant,
    ...
    if (!instant.isRequested()) {
      ...
      allRollbackStats = new RollbackExecutor(metaClient, config).performRollback(conf, instant, rollbackRequests);
    }
  ...
  private List<RollbackRequest> generateRollbackRequests(HoodieInstant instantToRollback)
    ...
    for(int i=0; i<partitions.size(); i++) {
      String partitionPath = partitions.get(i);
    ...
    return partitionRollbackRequests;
  }
  ...
  public void finalizeWrite(String instantTs, List<HoodieWriteStat> stats)
    ...
    super.finalizeWrite(instantTs, stats);
  }
