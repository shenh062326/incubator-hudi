...
import org.apache.hub.exception.HoodieRollbackException;
...
import static org.apache.hub.table.RollbackRequest.RollbackAction.APPEND_ROLLBACK_BLOCK;
import static org.apache.hub.table.RollbackRequest.RollbackAction.DELETE_DATA_AND_LOG_FILES;
import static org.apache.hub.table.RollbackRequest.RollbackAction.DELETE_DATA_FILES_ONLY;
...
public class RollbackExecutor implements Serializable {
   ...
   */
  public List<HoodieRollbackStat> performRollback(Configuration conf, HoodieInstant instantToRollback,
    ...
    Map<String, List<RollbackRequest>> map1 =
        rollbackRequests.stream().collect(Collectors.groupingBy(RollbackRequest::getPartitionPath));

    List<HoodieRollbackStat> res = new ArrayList<>();
    for (Map.Entry<String, List<RollbackRequest>> entry : map1.entrySet()) {
      List<RollbackRequest> rollbackRequests1 = entry.getValue();

      HoodieRollbackStat mergeHoodieRollbackStat = new HoodieRollbackStat(
          entry.getKey(), new ArrayList<>(), new ArrayList<>(), new HashMap<FileStatus, Long>());
      HoodieRollbackStat hoodieRollbackStat = mergeHoodieRollbackStat;

      for (int i = 0; i < rollbackRequests1.size(); i++) {
        RollbackRequest rollbackRequest = rollbackRequests1.get(i);
        final Map<FileStatus, Boolean> filesToDeletedStatus = new HashMap<>();

        if (rollbackRequest.getRollbackAction().equals(DELETE_DATA_FILES_ONLY)) {
        ...
        } else if (rollbackRequest.getRollbackAction().equals(DELETE_DATA_AND_LOG_FILES)) {
          deleteCleanedFiles(metaClient, config, filesToDeletedStatus, rollbackRequest.getPartitionPath(), filter);
          hoodieRollbackStat =
              HoodieRollbackStat.newBuilder().withPartitionPath(rollbackRequest.getPartitionPath())
                  .withDeletedFileResults(filesToDeletedStatus).build();
        } else if (rollbackRequest.getRollbackAction().equals(APPEND_ROLLBACK_BLOCK)) {
          Writer writer = null;
          try {
          ...
          try {
            filesToNumBlocksRollback.put(metaClient.getFs()
                .getFileStatus(Preconditions.checkNotNull(writer).getLogFile().getPath()), 1L);
          } catch (IOException io) {
            throw new HoodieRollbackException("Failed to get fileStatus " +
                Preconditions.checkNotNull(writer).getLogFile().getPath(), io);
          }
          hoodieRollbackStat =
              HoodieRollbackStat.newBuilder().withPartitionPath(rollbackRequest.getPartitionPath())
      ...
      mergeHoodieRollbackStat = mergeRollbackStat(mergeHoodieRollbackStat, hoodieRollbackStat);
      res.add(mergeHoodieRollbackStat);
    }
    return res;
  }
  ...
  private Map<FileStatus, Boolean> deleteCleanedFiles(HoodieTableMetaClient metaClient, HoodieWriteConfig config,
                                                      Map<FileStatus, Boolean> results, String partitionPath, PathFilter filter) {
