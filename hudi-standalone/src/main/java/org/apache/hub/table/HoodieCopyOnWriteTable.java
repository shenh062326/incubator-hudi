...
import org.apache.hub.exception.HoodieUpsertException;
import org.apache.hub.func.CopyOnWriteLazyInsertIterable;
import org.apache.hub.func.ParquetReaderIterator;
import org.apache.hub.func.SparkBoundedInMemoryExecutor;
import org.apache.hub.io.HoodieCleanHelper;
import org.apache.hub.io.HoodieCreateHandle;
import org.apache.hub.io.HoodieMergeHandle;
...
public class HoodieCopyOnWriteTable<T extends HoodieRecordPayload> extends HoodieTable<T> {
  ...
  public HoodieCopyOnWriteTable(HoodieWriteConfig config, Configuration conf) {
    super(config, conf);
  }
  ...
  private static Boolean deleteFileAndGetResult(FileSystem fs, String deletePathStr) {
    ...
    try {
      ...
      LOG.warn("Failed on delete path :" + deletePath, ioe);
      return false;
    }
  }
  ...
  @Override
  public HoodieCompactionPlan scheduleCompaction(Configuration conf, String commitTime) {
    throw new HoodieNotSupportedException("Compaction is not supported from a CopyOnWrite table");
  }

  @Override
  public List<WriteStatus> compact(Configuration conf, String compactionInstantTime,
   ...
   */
  @Override
  public HoodieCleanerPlan scheduleClean(Configuration conf) {
    try {
      ...
      Map<String, List<String>> cleanOps = partitionsToClean.stream().map(
          partitionPathToClean -> Pair.of(partitionPathToClean, cleaner.getDeletePaths(partitionPathToClean))
   ...
   */
  @Override
  public List<HoodieCleanStat> clean(Configuration conf, HoodieInstant cleanInstant, HoodieCleanerPlan cleanerPlan) {
    ...
    Map<String, List<Tuple2<String, String>>> groupByPartition =
        cleanerPlan.getFilesToBeDeletedPerPartition().entrySet().stream().flatMap(
            x -> x.getValue().stream().map(y -> new Tuple2<>(x.getKey(), y))
        ).collect(Collectors.groupingBy(Tuple2::_1));

    Optional<List<Tuple2<String, PartitionCleanStat>>> partitionCleanStatsOpt =
        groupByPartition.entrySet().stream().map(entry -> {
          Map<String, PartitionCleanStat> partitionCleanStatMap = new HashMap<>();

          FileSystem fs = this.getMetaClient().getFs();
          Path basePath = new Path(this.getMetaClient().getBasePath());
          Iterator<Tuple2<String, String>> iter = entry.getValue().iterator();
          while (iter.hasNext()) {
    ...
    List<Tuple2<String, PartitionCleanStat>> partitionCleanStats = null;
    if (partitionCleanStatsOpt.isPresent()) {
      partitionCleanStats = partitionCleanStatsOpt.get();
    } else {
      partitionCleanStats = new ArrayList<>();
    }
  ...
  @Override
  public List<HoodieRollbackStat> rollback(Configuration conf, HoodieInstant instant, boolean deleteInstants)
      throws IOException {
    ...
    if (!instant.isRequested()) {
      ...
      stats = new RollbackExecutor(metaClient, config).performRollback(conf, instant, rollbackRequests);
    }
