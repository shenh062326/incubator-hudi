...
public abstract class AbstractHoodieWriteClient<T extends HoodieRecordPayload> extends AbstractHoodieClient {
  ...
  protected AbstractHoodieWriteClient(Configuration conf, HoodieIndex index, HoodieWriteConfig clientConfig) {
    super(conf, clientConfig);
  ...
  protected AbstractHoodieWriteClient(Configuration conf, HoodieIndex index, HoodieWriteConfig clientConfig,
    ...
    super(conf, clientConfig, timelineServer);
    this.metrics = new HoodieMetrics(config, config.getTableName());
    this.index = index;
  }
   ...
   */
  public boolean commit(String commitTime, List<WriteStatus> writeStatuses) {
  ...
  protected List<WriteStatus> updateIndexAndCommitIfNeeded(List<WriteStatus> writeStatus, HoodieTable<T> table,
      String commitTime) {
    Timer.Context indexTimer = metrics.getIndexCtx();
    // Update the index back
    List<WriteStatus> statuses = index.updateLocation(writeStatus, table);
    metrics.updateIndexMetrics(UPDATE_STR, metrics.getDurationInMs(indexTimer == null ? 0L : indexTimer.stop()));
    // Trigger the insert and collect statuses
    commitOnAutoCommit(commitTime, writeStatus, table.getMetaClient().getCommitActionType());
    return writeStatus;
  }

  protected void commitOnAutoCommit(String commitTime, List<WriteStatus> resultRDD, String actionType) {
  ...
  private boolean commit(String commitTime, List<WriteStatus> writeStatuses,
  ...
  private void updateMetadataAndRollingStats(String actionType, HoodieCommitMetadata metadata,
    ...
    try {
      // Create a Hoodie table which encapsulated the commits and files visible
      HoodieTable table = HoodieTable.getHoodieTable(createMetaClient(true), config, conf);
  ...
  protected void rollbackInternal(String commitToRollback) {
    ...
    try {
      ...
      HoodieTable<T> table = HoodieTable.getHoodieTable(
          createMetaClient(true), config, conf);
  ...
  protected List<HoodieRollbackStat> doRollbackAndGetStats(final HoodieInstant instantToRollback) throws
    ...
    List<HoodieRollbackStat> stats = table.rollback(conf, instantToRollback, true);
