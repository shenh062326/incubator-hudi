...
public abstract class AbstractHoodieClient implements Serializable, AutoCloseable {
  ...
  protected AbstractHoodieClient(Configuration conf, HoodieWriteConfig clientConfig) {
    this(conf, clientConfig, Option.empty());
  }

  protected AbstractHoodieClient(Configuration conf, HoodieWriteConfig clientConfig,
      Option<EmbeddedTimelineService> timelineServer) {
    this.fs = FSUtils.getFs(clientConfig.getBasePath(), conf);
    this.conf = conf;
  ...
  private synchronized void startEmbeddedServerView() {
    if (config.isEmbeddedTimelineServerEnabled()) {
      if (!timelineServer.isPresent()) {
        ...
        timelineServer = Option.of(new EmbeddedTimelineService(conf,
  ...
  protected HoodieTableMetaClient createMetaClient(boolean loadActiveTimelineOnLoad) {
    return ClientUtils.createMetaClient(conf, config, loadActiveTimelineOnLoad);
  }
}
