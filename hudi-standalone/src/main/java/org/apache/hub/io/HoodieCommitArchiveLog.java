...
public class HoodieCommitArchiveLog {
   ...
   */
  public boolean archiveIfRequired(final Configuration conf) throws IOException {
    try {
      List<HoodieInstant> instantsToArchive = getInstantsToArchive(conf).collect(Collectors.toList());
  ...
  private Stream<HoodieInstant> getInstantsToArchive(Configuration conf) {
