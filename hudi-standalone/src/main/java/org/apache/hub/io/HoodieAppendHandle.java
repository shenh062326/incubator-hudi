...
import org.apache.hub.exception.HoodieAppendException;
...
public class HoodieAppendHandle<T extends HoodieRecordPayload> extends HoodieWriteHandle<T> {
  ...
  private void init(HoodieRecord record) {
    if (doInit) {
      ...
      //TODO should improve averageRecordSize
  ...
  private Option<IndexedRecord> getIndexedRecord(HoodieRecord<T> hoodieRecord) {
    ...
    try {
      ...
      if (avroRecord.isPresent()) {
        ...
        String seqId =
            HoodieRecord.generateSequenceId(instantTime, 0, recordIndex.getAndIncrement());
  ...
  private void flushToDiskIfRequired(HoodieRecord record) {
    ...
    if (numberOfRecords >= (int) (maxBlockSize / averageRecordSize)) {
      ...
      // TODO improve averageRecordSize
